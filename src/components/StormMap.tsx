...
const PROB_COLORS: Record<string, string> = {
  "2": "#00bb00", "5": "#8bce00", "10": "#ffcc00", "15": "#ff9900", "30": "#ff0000", "45": "#ff00ff", "60": "#912cee",
};
const CITIES = [
  { name: "Denver", lat: 39.74, lng: -104.99 }, { name: "Oklahoma City", lat: 35.47, lng: -97.52 },
  { name: "Dallas", lat: 32.78, lng: -96.8 }, { name: "Kansas City", lat: 39.1, lng: -94.58 },
  { name: "Omaha", lat: 41.26, lng: -95.94 }, { name: "Minneapolis", lat: 44.98, lng: -93.27 },
  { name: "Chicago", lat: 41.88, lng: -87.63 }, { name: "St. Louis", lat: 38.63, lng: -90.2 },
  { name: "Memphis", lat: 35.15, lng: -90.05 }, { name: "Atlanta", lat: 33.75, lng: -84.39 },
  { name: "Nashville", lat: 36.16, lng: -86.78 }, { name: "Lubbock", lat: 33.58, lng: -101.86 },
  { name: "Amarillo", lat: 35.22, lng: -101.83 }, { name: "Wichita", lat: 37.69, lng: -97.34 },
  { name: "Des Moines", lat: 41.59, lng: -93.62 }, { name: "Sioux Falls", lat: 43.55, lng: -96.7 },
];
...
function getCentroid(geometry: any): [number, number] | null {
  ...
  if (geometry.type === "Polygon") coords = geometry.coordinates[0] || [];
    ...
    for (const g of geometry.geometries || []) { const c = getCentroid(g); if (c) return c; }
    return null;
  } else return null;
  if (!coords.length) return null;
  let lat = 0, lng = 0, n = 0;
  for (const c of coords) { if (Array.isArray(c) && c.length >= 2) { lng += c[0]; lat += c[1]; n++; } }
  return n ? [lat / n, lng / n] : null;
}
function hasDrawableGeometry(geometry: any): boolean {
  if (!geometry) return false;
  if (geometry.type === "GeometryCollection") return (geometry.geometries || []).some((g: any) => hasDrawableGeometry(g));
  return geometry.type === "Polygon" || geometry.type === "MultiPolygon" || geometry.type === "Point";
}
function severityColor(severity?: string): string {
  const s = (severity || "").toLowerCase();
  if (s === "extreme") return "#ff2d2d"; if (s === "severe") return "#ff5c5c";
  if (s === "moderate") return "#ff9f43"; if (s === "minor") return "#ffd166"; return "#52e0d0";
}
...
function isDangerousEvent(event?: string): boolean {
  ...
  return e.includes("tornado warning") || e.includes("flash flood warning") || e.includes("hurricane warning") || e.includes("extreme");
}
function btnStyle(active: boolean): React.CSSProperties {
  return {
    background: active ? "rgba(217,255,74,0.18)" : "rgba(5,9,11,0.88)",
    border: active ? "1px solid rgba(217,255,74,0.5)" : "1px solid rgba(184,221,225,0.22)",
    borderRadius: 8, padding: "5px 10px", fontSize: 10,
    color: active ? "#d9ff4a" : "#8fa6a8", fontWeight: 700, letterSpacing: "0.04em", cursor: "pointer", textAlign: "left",
  };
}

export default function StormMap({ chaseMode = false }: { chaseMode?: boolean }) {
  const mapHeight = chaseMode ? "min(72vh, 720px)" : 420;
  ...
  const [showYesterday, setShowYesterday] = useState(() => {
    try {
      return localStorage.getItem("stormiq-prefer-yesterday") === "1";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    const map = L.map(mapRef.current, { center: [39.5, -98.0], zoom: 4, zoomControl: true, minZoom: 3, maxZoom: 12 });
    ...
    const radar = L.tileLayer("https://mesonet.agron.iastate.edu/cache/tile.py/1.0.0/nexrad-n0q-900913/{z}/{x}/{y}.png", { attribution: "Radar © Iowa State Mesonet / NWS NEXRAD", opacity: 0.55, zIndex: 200, maxZoom: 12 });
    radar.addTo(map);
    ...
    CITIES.forEach((c) => {
      L.circleMarker([c.lat, c.lng], { radius: 4, color: "#8fa6a8", fillColor: "#edf8f7", fillOpacity: 0.85, weight: 1 })
        .bindTooltip(c.name, { permanent: true, direction: "right", offset: [6, 0], className: "city-label" })
        .addTo(cityGroup);
    });
    cityGroup.addTo(map);
    ...
    async function loadLayers() {
      ...
      if (!m || !dynamicRef.current || !tornLayerRef.current || !hailLayerRef.current) return;
      dynamicRef.current.clearLayers();
      tornLayerRef.current.clearLayers();
      hailLayerRef.current.clearLayers();
      let outlookCount = 0, alertCount = 0, hailCount = 0, tornCount = 0;
      ...
      try {
        ...
        if (res.ok) {
          ...
          (geo?.features || []).forEach((f: any) => {
            ...
            L.geoJSON(f, { style: { color, weight: 2, fillColor: color, fillOpacity: label === "TSTM" ? 0.08 : 0.22 } })
              .bindPopup(`<strong>SPC Day 1 · ${label}</strong>`).addTo(dynamicRef.current!);
            outlookCount++;
          });
        }
      } catch (e) { console.warn("SPC cat failed", e); }

      try {
        const res = await fetch("https://www.spc.noaa.gov/products/outlook/day1otlk_torn.nolyr.geojson");
        if (res.ok) {
          const geo = await res.json();
          (geo?.features || []).forEach((f: any) => {
            ...
            if (!hasDrawableGeometry(f.geometry)) return;
            L.geoJSON(f, { style: { color, weight: 1.5, fillColor: color, fillOpacity: 0.2, dashArray: "4 3" } })
              .bindPopup(`<strong>Tornado probability · ${label}%</strong>`).addTo(tornLayerRef.current!);
            tornCount++;
          });
        }
      } catch (e) { console.warn("SPC torn failed", e); }
      ...
      try {
        ...
        if (res.ok) {
          ...
          (geo?.features || []).forEach((f: any) => {
            ...
            const color = PROB_COLORS[label] || "#00aa00";
            if (!hasDrawableGeometry(f.geometry)) return;
            L.geoJSON(f, { style: { color, weight: 2, fillColor: color, fillOpacity: 0.22, dashArray: "2 4" } })
              .bindPopup(`<strong>Hail probability · ${label}%</strong>`).addTo(hailLayerRef.current!);
            hailCount++;
          });
        }
      } catch (e) { console.warn("SPC hail failed", e); }
      ...
      try {
        ...
        if (res.ok) {
          ...
          for (const f of data?.features || []) {
            ...
            const interesting = event.includes("warning") || event.includes("watch") || event.includes("tornado") || event.includes("thunderstorm") || event.includes("flood") || event.includes("hurricane") || event.includes("tropical") || event.includes("winter") || event.includes("wind");
            ...
            const detail = `<div style="min-width:200px;font-family:system-ui;line-height:1.4"><strong>${eventName}</strong><br/><span style="font-size:12px;color:#333">${props.headline || props.areaDesc || ""}</span><br/><span style="font-size:11px;color:#555">Severity: ${props.severity || "—"}</span><div style="margin-top:8px;font-size:11px;color:#666">Tap marker for route panel</div></div>`;
            if (showPolygonsRef.current && hasDrawableGeometry(f.geometry) && f.geometry?.type !== "Point") {
              try {
                L.geoJSON(f, { style: { color, weight: event.includes("warning") ? 2.5 : 1.5, fillColor: color, fillOpacity: event.includes("warning") ? 0.22 : 0.12 } }).bindPopup(detail).addTo(dynamicRef.current!);
              } catch { /* skip */ }
            }
            if (center) {
              const [lat, lng] = center;
              const marker = L.circleMarker(center, { radius: event.includes("warning") ? 8 : 6, color, fillColor: color, fillOpacity: 0.95, weight: 2 }).addTo(dynamicRef.current!);
              marker.bindPopup(detail);
              marker.on("click", () => setSelected({ lat, lng, title: eventName, dangerous }));
            }
            alertCount++;
            if (alertCount >= 200) break;
          }
        }
      } catch (e) { console.warn("NWS failed", e); }
      ...
      const hailNote = hailCount === 0 ? " · hail none" : ` · hail ${hailCount}`;
      const tornNote = tornCount === 0 ? "" : ` · torn ${tornCount}`;
    ...
    if (!document.getElementById("stormiq-city-css")) {
      ...
      style.textContent = `.city-label{background:transparent!important;border:none!important;box-shadow:none!important;color:#b8c8ca!important;font-size:10px!important;font-weight:600!important;text-shadow:0 1px 2px #000}.city-label::before{display:none!important}`;
  ...
  async function drawRoute() {
    ...
    try {
      ...
      const line = L.polyline([[home.lat, home.lng], [dest.lat, dest.lng]], { color: "#d9ff4a", weight: 3, opacity: 0.85, dashArray: "6 6" }).addTo(map);
      routeLayerRef.current = line;
      map.fitBounds(line.getBounds(), { padding: [36, 36] });
      setRouteInfo("Straight path (routing unavailable)");
    }
  }
  ...
  function openMaps() {
    if (!selected) return;
    window.open(mapsUrl(selected.lat, selected.lng, loadHomeBase()), "_blank", "noopener");
  }

  useEffect(() => {
    const radar = radarLayerRef.current; const map = mapInstance.current;
    if (!radar || !map) return;
    if (radarOn) { if (!map.hasLayer(radar)) radar.addTo(map); }
    else if (map.hasLayer(radar)) map.removeLayer(radar);
  }, [radarOn]);
  useEffect(() => {
    const map = mapInstance.current; if (!map) return;
    if (showTorn && tornLayerRef.current && !map.hasLayer(tornLayerRef.current)) tornLayerRef.current.addTo(map);
    else if (!showTorn && tornLayerRef.current && map.hasLayer(tornLayerRef.current)) map.removeLayer(tornLayerRef.current);
  }, [showTorn]);
  useEffect(() => {
    const map = mapInstance.current; if (!map) return;
    if (showHail && hailLayerRef.current && !map.hasLayer(hailLayerRef.current)) hailLayerRef.current.addTo(map);
    else if (!showHail && hailLayerRef.current && map.hasLayer(hailLayerRef.current)) map.removeLayer(hailLayerRef.current);
  }, [showHail]);
  useEffect(() => {
    const map = mapInstance.current as any; if (!map?._cityGroup) return;
    if (showCities) map._cityGroup.addTo(map); else map.removeLayer(map._cityGroup);
  }, [showCities]);
  useEffect(() => {
    const map = mapInstance.current; if (!map) return;
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: mapHeight, minHeight: mapHeight }}>
      ...
      <div style={{ position: "absolute", top: 12, left: 12, zIndex: 1000, display: "flex", flexDirection: "column", gap: 6 }}>
        <button onClick={() => setRadarOn((v) => !v)} style={btnStyle(radarOn)}>{radarOn ? "RADAR ON" : "RADAR OFF"}</button>
        <button onClick={() => setShowTorn((v) => !v)} style={btnStyle(showTorn)}>{showTorn ? "TORNADO ON" : "TORNADO OFF"}</button>
        <button onClick={() => setShowHail((v) => !v)} style={btnStyle(showHail)}>{showHail ? "HAIL ON" : "HAIL OFF"}</button>
      ...
      {selected && (
        <div style={{ position: "absolute", left: 12, right: 12, bottom: 16, zIndex: 1100, background: "rgba(5,9,11,0.94)", border: selected.dangerous ? "1px solid rgba(255,92,92,0.55)" : "1px solid rgba(217,255,74,0.35)", borderRadius: 12, padding: "12px 14px" }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={drawRoute} style={{ background: "rgba(217,255,74,0.16)", border: "1px solid rgba(217,255,74,0.45)", color: "#d9ff4a", borderRadius: 8, padding: "8px 12px", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>SHOW ROUTE</button>
            <button onClick={openMaps} style={{ background: "rgba(82,224,208,0.12)", border: "1px solid rgba(82,224,208,0.35)", color: "#52e0d0", borderRadius: 8, padding: "8px 12px", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>OPEN IN MAPS</button>
            <button onClick={clearRoute} style={{ background: "transparent", border: "1px solid rgba(184,221,225,0.2)", color: "#8fa6a8", borderRadius: 8, padding: "8px 12px", fontWeight: 600, fontSize: 11, cursor: "pointer" }}>CLEAR</button>
          </div>
        </div>
      )}
    </div>
